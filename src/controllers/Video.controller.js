...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  const videoFile = await uploadOnCloudinary(videosFileLocalPath);
  const thumbnail = await uploadOnCloudinary(thumbnailFileLocalPath);

  if (!videoFile) {
    throw new (400, "Video file is not found")();
  }

  if (!thumbnail) {
    throw new (400, "thumbnail is not found")();
  }
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (video?.owner?.toString() !== req.user?._id.toString()) {
    throw new ApiError(
      400,
      "you can't edit this video as you're not the owner"
    );
  }

  // deleting old thumbnail and update with new one
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updateVideo, "vide updated successfully"));
});
